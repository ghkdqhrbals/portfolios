...
title: API 최적화 방법
date: 2025-11-11
summary: "최근 API 응답 최적화 관련 질문을 받았꼬 해당 방법에 대해 진짜 시작부터 끝까지 하나의 플로우로 정리해봤다. 일단 API 는 유저 정보를 불러오는 API 이며 내부망 API 2번 및 DB 1번 호출이 포함되어 있다고 가정한다."
tags: ["java", "backend", "virtual-thread", "grpc", "performance", "gzip"]
auto_translate: true
translation_en: |
  ...
  * Check whether indexes are configured properly for DB select queries with `explain`.
    * If an index is on a low-cardinality field, remove it or redesign it.
