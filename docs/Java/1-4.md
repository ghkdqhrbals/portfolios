...
date: 2022-04-25
summary: "Introduction Previously, the user notification logic was embedded directly in our business logic, with a failover sequence like PUSH → SMS → Alimtalk."
auto_translate: true
translation_en: |
  ## Introduction
  ...
  ![a](/portfolios/assets/1-4/2025-05-01-20-18-11.png)
  ...
  **Too expensive.** So we pivoted: "Let’s build an MQ ourselves using Redis Stream, which we were already using in production."
  ...
  This post explains how Redis Stream's PEL, `XAUTOCLAIM`, deterministic hash assignment, locks, DLQ handling, and custom consumer heartbeat management can be combined to build a lightweight MQ. Redis lacks Kafka-style partitioning, disk persistence, and built-in rebalancing, but for lightweight notification events it was fast, cheap, and controllable enough.
parent: Server
---
...
![a](/portfolios/assets/1-4/2025-05-02-16-13-47.png)
