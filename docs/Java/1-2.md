...
date: 2022-12-02
summary: "Kafka 아키텍처 용어 설명 Producer : 이벤트를 보내는 주체 Consumer: 토픽의 파티션에 저장되어 있는 메시지를 소비(consume)하는 역할. Consumer Group : 하나의 Topic의 모든 파티션들을 구독하는 consumer 그룹."
auto_translate: true
translation_en: |
  # Kafka
  ...
  * **Consumer**: consumes messages stored in topic partitions.
  * **Consumer Group**: a group of consumers that subscribe to all partitions of one topic.
  * **Partition**: storage for messages received from Producers.
    * Leader Partition: the partition that actually communicates with Producer/Consumer.
    * Follower Partition: replicates and stores data from the Leader Partition. If the Leader fails, a Follower can be promoted.
  * **Broker** = Kafka server: manages topics and partitions.
    * Controller: one broker takes this role and promotes a follower partition when a leader partition has a problem.
    * Coordinator: one broker takes this role and matches partitions to other Consumers when messages are not consumed due to failure.
  * **Topic**: the subject of messages.
  * **Offset**: message position inside a partition.
  * **Kafka Cluster**: a group of Kafka Brokers.
  * **Event**: same as Message.
  * **Zookeeper**: server that stores and **manages** Kafka metadata.

  * **Replication Factor**: determines how many Brokers process each Topic.
  > If three Brokers are configured, another Broker can process work when one Broker fails. Broker settings should depend on topic importance.

  ## How it works

  1. The client sends a message to the Producer.
  2. The Producer stores messages up to the batch size in the leader of a specific topic partition.
     * The partition leader and followers are grouped as ISR, In Sync Replica, and synchronize data.
  3. Consumers connected to each broker read messages when needed and handle events. The Broker records metadata for delivered events.

  ## Advantages and strategies

  * Leader/Follower replication helps stable operation and prevents SPOF.
  * Processing data by Topic can reduce overhead.
  * Exactly Once delivers and processes a message exactly once, but overhead is high.
  * At Least Once delivers/processes at least once, so idempotency is required.
  * At Most Once delivers/processes at most once, so message loss can occur.

  ## Notes

  Since Consumers can read Broker messages multiple times, idempotency must be considered. The same applies to Producers.
...
## 메세지 처리 전략 3가지
![img.png](/portfolios/assets/1-2/img.png)
