...
date: 2023-09-10
summary: "JPQL JOIN 들이 헷갈리는 부분이 있어서 정리해보았습니다. JPQL 에는 총 4 가지 JOIN 방식이 존재합니다. 그리고 이 JOIN 방식에 따라 영속성 컨텍스트로의 로딩 전략이 달라집니다. JPQL 에서 JOIN 을 사용하려면 연관관계를 엔티티에 설정해주어야만 합니다."
auto_translate: true
translation_en: |
