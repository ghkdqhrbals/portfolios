...
date: 2025-10-09
summary: "용량제한 90% 가 넘으면 경고가 뜨고, 95% 가 넘으면 읽기전용으로 바뀐다. 롤오버로 새로운 인덱스 생성하는거 굳이 안써도 됨."
tags: ["java", "backend", "elasticsearch", "index", "kubernetes"]
auto_translate: true
translation_en: |
  ```
  ...
  When disk usage goes over the 90% limit, a warning appears. When it goes over 95%, the index changes to read-only.
  ...
  There is no need to use rollover to create a new index. Since we already create a new index per day and keep it for 30 days, it is enough to delete data older than 30 days from the existing `2025-10-10` style index.
  ...
  Our indexes are created with the `k8s-2025.09.11` pattern. To apply a policy that deletes all of these indexes after 30 days, create the template below.
