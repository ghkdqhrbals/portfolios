...
title: Kafka에 대한 고찰
date: 2022-12-19
summary: "Request-response (HTTP) vs. event streaming (Kafka) 정리 글 event streaming 2 페이스북같은 경우, 유저 프로파일을 변경했을 때 여러 다른서버들과 연동이 일어난다."
tags: ["java", "backend", "kafka", "elasticsearch", "database", "index", "http"]
auto_translate: true
translation_en: |
  1. Summary post about Request-response (HTTP) vs. event streaming (Kafka)
  ...
  In a Facebook-like case, when a user profile is changed, several other servers are involved. From the perspective of those servers, they do not need to return directly to the user immediately.
