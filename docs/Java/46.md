...
sequenceDiagram
    ...
    participant RAM as API Local Memory<br/>(eventId → Future)
    ...
    C->>API: POST /orders (주문 요청)
    API->>API: eventId 생성
    API->>RAM: Future 저장 (eventId → CompletableFuture)
    API->>R: XADD create-order {eventId, userId, payload}
    API-->>C: 202 Accepted<br/>{eventId}

    Note over API: Controller 스레드는 Future.await()

    R->>CS: Consumer reads event (create-order)
    CS->>DB: event.state = PROCESSING 업데이트
    CS->>CS: 주문/장바구니 로직 처리

    alt 처리 성공
        CS->>DB: event.state = COMPLETED, result 저장
    else 처리 실패
        CS->>DB: event.state = FAILED, error 저장
    end

    CS->>RS: GET sse:{userId} → serverId

    alt serverId == CS 서버 (예: Server-A)
        CS->>S1: pushEvent(eventId, result)
        S1->>C: SSE/WebSocket: 주문 처리 완료(result)
        S1->>RAM: eventId Future.complete(result)
    else serverId != CS 서버 (예: Server-B)
        CS->>GRPC: pushEvent(serverId, eventId, result)
        GRPC->>S2: 전달
        S2->>C: SSE/WebSocket: 주문 처리 완료(result)
        S2->>RAM: eventId Future.complete(result)
    end

    API-->>C: HTTP 최종 응답(Future 완료 후)
