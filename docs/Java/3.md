...
parent: Server
date: 2023-04-08
summary: "{: .important } Reactive 프로그래밍에서는 Spring Security 설정이 기본과는 다릅니다. 별로 레퍼런스가 없길래 아래와 같이 코드를 공유합니다. 1. LoginResponseDto 2. LoginRequestDto 3."
tags: ["java", "backend", "jwt"]
auto_translate: true
translation_en: |
  {: .important }
  > **In reactive programming, Spring Security configuration is different from the basic servlet setup. There were not many references, so I am sharing the code below.**

  This post contains a WebFlux Spring Security + JWT setup.

  ## Main components

  1. `LoginResponseDto`: response DTO that returns the JWT token.
  2. `LoginRequestDto`: request DTO containing username and password.
  3. `CustomUserDetails`: implements `UserDetails` and converts permissions into `SimpleGrantedAuthority`.
  4. `LoginService`: authenticates with `ReactiveAuthenticationManager`, creates a token with `JwtTokenProvider`, and adds it to the `Authorization` response header.
  5. `ReactiveSecurityConfig`: configures `ServerHttpSecurity`, disables CSRF/form login/http basic, adds a JWT filter, and configures route authorization.
  6. `JwtTokenProvider`: creates, parses, and validates JWT tokens.
  7. `JwtTokenAuthenticationFilter`: reads the `Bearer` token from the header, validates it, and writes `Authentication` into `ReactiveSecurityContextHolder`.

  The important difference from servlet security is that WebFlux uses `SecurityWebFilterChain`, `WebFilter`, `ReactiveAuthenticationManager`, and `ReactiveSecurityContextHolder`.
nav_order: 6
---
