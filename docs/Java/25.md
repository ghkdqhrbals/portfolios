...
title: 자동화 리스트
date: 2025-01-18
summary: "1. 빌드 & 모니터링 자동화 MODULE TO BUILD 변경된 파일 목록들 가져와서 공통 모듈이 변경되면 해당 모듈로부터 파생된 다른 모듈들 전부 deploy list 에 넣기. 만약 특정 하위 모듈만 변경되면 해당 모듈만 list 에 추가."
auto_translate: true
translation_en: |
  ## 1. Build & monitoring automation

  #### MODULE TO BUILD

  * Get the list of changed files. If a common module changes, put all modules derived from that module into the deploy list.
  * If only a specific submodule changes, add only that module to the list.
    Additional repeated work happened here too: `the build machine keeps dying` -> `reboot + run git runner start cmd` -> `check idle again`.
    This was annoying, so I automated it like this: automated instance reboot and git runner start command execution with AWS Lambda, reboot every day with cronjob, connected AWS API Gateway so sending a REST API GET request automatically reboots it. So it can be recovered with one button click.
    But HTTP API Gateway cannot limit inbound traffic with ACL, so it looks like I should connect ALB and Lambda instead.

  #### JIB

  Use Jib so Docker image build + `./gradlew build` + image push can be done with just one command, `./gradlew jib`, without a Dockerfile. But with Jib, there was a problem where the hash value changed when files were copied, so normal COPY did not work. I changed it so files are compressed with tar before COPY.

  #### GRAFANA

  To check cronjob batch status, Grafana pulls data from the batch DB and displays status by time. But if a job is configured not to insert into the job repo, it cannot be read. So for more accurate monitoring, it would be better to connect with Prometheus so Kubernetes cronjob logs can be read and stored.
  I am additionally looking for a plugin that sends a REST API request when a button is clicked.

  ## 2. Slack List Notification(upload to actions marketplace)

  ![img_2.png](/portfolios/assets/25/img_4.png)

  When using GitHub Actions, there are many times when intermediate steps need to be notified through Slack. The message sending script is too annoying, so I made it as a common module and used it. Even that was annoying because I had to fetch it and run `chmod +x` to add execute permission.

  So I uploaded it to GitHub Actions Marketplace and changed it so it can be used easily like below. ~~However, modifying it can be a bit annoying.~~ It seems better to upload only scripts that are truly shared commonly to the marketplace.
  ...
  ```yaml
    - name: Send Message to Slack with Building Modules
      ...
      with:
        ...
        message-title: 'Manual dev deployment started (Version: ${{ steps.set-version.outputs.version }})'
        color: '#3bb143'
  ```

  Draft repo: [https://github.com/ghkdqhrbals/slack-list](https://github.com/ghkdqhrbals/slack-list)
  > Create `actions.yaml` at the Git repo root, create a draft, release it, and then import that action with `uses`.

  ## 3. K8S automatic rollout tracking, to catch image pullback off errors and runtime errors

  ![img_2.png](/portfolios/assets/25/img_2.png)

  **Originally, when running `kubectl apply -f ...deployment.yaml`, runtime errors that happen when the pod starts could not be handled.**
  So I wanted to send a Slack notification when retries exceeded a certain number.

  But complexity and maintenance time are proportional, so I simply set a time limit on rollout duration and made it fail if the limit is exceeded.

  > It is better to set rollout status observation job concurrency to one. Otherwise, when another job comes in, adding `cancel-in-progress: true` to stop the old job and run the latest job is also good.
  ...
  **As too many messages accumulated, there was a need to bundle messages together.** Sometimes I need to update an existing message or add extra information as a comment. I made a script that can be used in that case.
  ...
  new_message="$2"
  channel="$3"
  timestamp_file=".slack_last_ts"

  # Slack message update function, replacing attachments.
  ...
  # Function to add a message as a comment.
  add_comment() {
    local ts="$1"
    local msg="$2"
  ...
  send_message() {
    ...
    # Extract timestamp after sending the message.
    local ts
...
![img_5.png](/portfolios/assets/25/img_5.png)
