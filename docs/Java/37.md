...
기존에 저희는 Mysql 를 main db 로, 멀티프렌차이즈를 목표로 운영하다보니 여러 다양한 스키마들이 필요했고 이를 위한 Mongodb 를 secondary db 로 운영하고 있었습니다. 그리고 신규 피처로 대량의 회원추출을 진행하기 위해 mysql slave 를 보는 것이 아닌, cdc 를 통해서 Mongodb 에 회원정보를 upsert 하는 작업을 진행하고 있습니다. MongoDB 는 실행 흐름이 눈에 보이고 개발자가 통제할 수 있기 때문이죠. 성능 유지보수 하기 좋아요. mysql 쿼리플랜은 옵티마이저가 알아서 만들거든요.
...
- source binload 세팅
    ...
    // source binload 세팅
    "source": {
      "version": "3.2.0.Final",         // Debezium 커넥터가 이벤트를 생성한 Debezium 버전
      "connector": "mysql",              // 사용된 커넥터 유형(여기선 MySQL 커넥터)
      "name": "mysql_sink",              // 커넥터(논리 서버) 이름. 토픽/키(prefix)로도 쓰임: {name}.{db}.{table}
      "ts_ms": 1755138551000,            // 소스 이벤트 시각(ms). MySQL의 binlog 이벤트 타임스탬프 기반
      "snapshot": "false",               // 스냅샷 여부: true(스냅샷 중), last(스냅샷 마지막 레코드), false(실시간 CDC), incremental(증분 스냅샷)
      "db": "gyumin",                  // 변경이 발생한 스키마 이름
      "sequence": null,                  // 일부 커넥터에서 사용되는 순번(Oracle/DB2 등). MySQL에선 보통 null
