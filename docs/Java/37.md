...
# 직접 구현한 Sink (CDC + ETL) 데이터 적재 모듈 관련 내용정리
...
- 참고: 단순 테이블은 findAndReplace(upsert)로 충분. 부모 문���의 배열을 수정해야 하는 경우(예: order_items)에는 안전한 필드 단위 .set 사용을 고려(toDotPath 활용)
...
## Indexes 넣어서 ETL 성능 최적화(update 할 떄 select 속도관련)
