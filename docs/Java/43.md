...
protobuf 는 좀 귀찮아진다. 코드 생성단계를 거쳐야 하고, 디버깅도 조금 어렵다. 기존 웹 상에서 따로 지원해줘야한다. 그래서 조금 까다롭다뿐이고 성능적 이점은 확실하니 대부분의 경우에서 gRPC 를 쓰는게 좋다. 특히 내부망에서 대용량 데이터를 주고받을 때는 아주 유리하다.