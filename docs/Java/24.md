...
date: 2024-12-30
summary: "보통 하나의 instance 에서 하나의 runner 만 추가하는 경우가 많습니다. runner 은 하나의 job 만 처리가능해서 matrix 를 사용하여 여러개의 job 을 병렬처리할 수 없죠. 디렉토리가 다르면 runner 를 여러개 추가할 수 있습니다."
auto_translate: true
translation_en: |
  Usually, many people add only one runner to one instance. A runner can process only one job, so even if you use a matrix, you cannot process multiple jobs in parallel.

  If the directories are different, multiple runners can be added. I did not want to manually split directories, install them, and so on for parallel job execution, so I brought in a script.

  ### 1. Script for adding multiple self-hosted runners to one build machine
  ...
  # Configure runner
  ./config.sh --url 'git repo address' \
      --token "$TOKEN" \
  ...
  After writing the script above, running it like below adds runners one by one.
  ...
  ![img.png](/portfolios/assets/24/img.png)

  ### 2. Script for removing a self-hosted runner
  ...
  ![img_1.png](/portfolios/assets/24/img_1.png)
parent: Server
nav_order: 21
---
