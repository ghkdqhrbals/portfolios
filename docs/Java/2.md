...
parent: Server
date: 2023-04-05
summary: "1. WebFlux 한줄 정의 WebFlux는 Reactive Programming 을 편하게 사용가능하게하는 Spring 제공 객체로, Pub/Sub 방식으로 이벤트 메세지를 주고받습니다."
tags: ["java", "backend"]
auto_translate: true
translation_en: |
  ## 1. One-line definition of WebFlux

  WebFlux is a Spring-provided object that makes Reactive Programming easier to use. It exchanges event messages in a Pub/Sub style.
  > Reactive Programming is an **asynchronous** programming paradigm centered on data streams and propagation of changes.

  ## 2. WebFlux + Future example

  The example uses `Sinks.Many<Object>` with `Sinks.many().replay().all()`. This allows multiple threads to access the Flux through the Sink. `replay().all()` means that when a new subscriber is added, all previous messages are replayed to it.

  The flow is:

  * Create a Sink.
  * Subscribe to the Sink's Flux from different threads through `CompletableFuture`.
  * Emit `First`, `Second`, `Third`, and `Forth` from different threads.
  * Existing subscribers receive newly emitted messages.
  * New subscribers receive previous messages because `replay().all()` is used.

  ## 3. Sinks methods

  * **many().multicast()**: transmits only newly pushed data to subscribers.
  * **many().unicast()**: similar to multicast, but buffers data pushed before the first subscriber registers.
  * **many().replay()**: replays a specified history size to new subscribers, then continues pushing live data.

  ## 4. Bifunction WebFlux

  `Flux.generate` creates values based on a state object. The example starts from `0`, emits `"3 x state = result"`, completes when state reaches `10`, and returns `state + 1` for the next iteration.
nav_order: 5
---
