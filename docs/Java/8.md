...
parent: Server
date: 2023-06-18
summary: "Gateway 지원 로드밸런싱 기술  : Default로 ReactiveLoadBalancer 을 사용하고, 이는 Round-Robin(default) 과 Random 이 두 가지를 지원합니다."
tags: ["java", "backend", "nginx"]
auto_translate: true
translation_en: |
  ## Load balancing technologies supported by Gateway
  ...
  By default, it uses `ReactiveLoadBalancer`, and this supports two strategies: `Round-Robin`, which is the default, and `Random`.
  If you want to apply load balancing like `ip_hash` in Nginx, refer to [custom load balancer](https://docs.spring.io/spring-cloud-commons/docs/current/reference/html/#custom-loadbalancer-configuration).

  ## Checking Gateway load balancing statistics

  * The following load-balancing-related statistics are exposed through Micrometer. Actuator needs to be implemented in the Gateway.
  ...
  >
  > Reference: [checking statistics](https://docs.spring.io/spring-cloud-commons/docs/current/reference/html/#loadbalancer-micrometer-stats-lifecycle)
nav_order: 10
---
