...
parent: 토스 러너스 하이 2기
date: 2025-12-31
summary: "redis 8.2 버전에선 lag 데이터를 같이 전달해주지만, 최신의 spring data redis 에선(3.5.x or 4.0.x) lag 정보를 파싱해주지 않아서 직접 lua script 로 lag 정보를 가져온 뒤 파싱해주어야합니다. spring data redis 4."
tags: ["toss", "backend", "redis-stream", "redis"]
---
