...
> 
> ngrinder 을 통한 csv 파일로부터 MTTFB 평균과 변동량, 변동률, p95, p99 측정하는 python 코드는 포스팅 맨 아래에 있습니다!
> 
> Notation : (🟢 for improvement, 🔴 for decline)
...
만약 HPA 파드 오토 스케일 아웃 없이 단일 파드로만 핸들링할 때 어떤 결과가 나올지 궁금하지 않나요? 그래서 실험해보았고 아래와 같은 결과가 나왔습니다. 실험 중 오류가 너무 많이 발생되서 자동으로 중지될 정도로 많은 오류를 반환합니다.
...
엄청나죠? 에러률은 50% 이상을 기록합니다. TPS 하위 1%, 5%는 거의 탈락되고 MTTFB 차이 평균이 약 3초 가량 나오는 것을 확인할 수 있었습니다. MTTFB 는 아주 10분동안 실행하도록 지시했지만 03:10 에 높은 에러율로 인해 종료되었습니다.
...
중간중간 TPS 가 감소하는 모습을 확 위의 그래프에서 확인할 수 있습니다. 역시 VUSER 가 100명일 때와 300일 때는 많이 다르네요ㅜㅜ. TPS 감소 이유는 아래의 그래프에서 원인을 파악할 수 있습니다.
