---
layout: default
title: 43. 서버성능 개선기록 - 🔴MTTFB 평균차이 - 40% 성능감소
...
우리가 봐야할 부분은 UserRedisSessionRepository 의 지연시간입니다! **이 지연시간이 다른 RDB 와 비교했을 때 너무나도 높았습니다**.

**아니 빠르게 데이터를 찾기위해 Redis remote in-memory 를 쓰는데, RDB 보다 성능이 낮다니요...** 

그래서 저는 Redis 의 성능을 높이기 위해 Lettuce 가 제공하는 connection pool 을 쓰기로 했습니다. **Lettuce CP(Connection Pool)의 default 값들을 수정**하면서 여러 성능지표들을 모아보았습니다.
...
- 기존(max-active:8, max-idle: 8, min-idle: 0) **= Original**
  ...
  * MTTFB 평균: 456.42 ms
  * MTTFB p95: 799.67 ms
  * MTTFB p99: 1130.67 ms
  * MTTFB 차이 평균 : 74.02 ms
  * MTTFB 평균적인 변동률: 15.27%
...
![image](https://github.com/ghkdqhrbals/spring-chatting-server/assets/29156882/5d693f5d-fea6-4498-809b-bffb8ae1c15c)

{: .highlight }
> 결과부터 말씀드리면 낮은 CP 가 오히려 성능지표가 더 좋게 나왔습니다!

CP 증가 시에도 여전히 Redis Repository Latency 가 다른 RDB Latency 보다 높습니다 😂 **그래도 살짝 감소한 모습은 확인할 수 있었습니다!** 하지만 오차범위 내라고 생각합니다.
...
- [x]  네트워크 bandwidth 부족문제 X    
  → 네트워크 1.5Mbps 만 써서 매우 충분 😟
- [x]  과도한 cpu usage 문제 X   
...
- [x]  메모리 fragementation ? X 아니면 메모리 사용량이 너무 높은가요?     
  ![image](https://github.com/ghkdqhrbals/spring-chatting-server/assets/29156882/3708690b-a74f-44e3-a18b-14ff61b1d674)
  → 잔여 메모리가 40% 이상 남아있습니다. 그래서 단편화는 제외했습니다.
- [x]  Key indexing 이 제대로 안되어있어서 찾기가 힘든가요? X      
...
- [x] Race Condition 문제인가요? X        
  Redis 는 싱글스레드로 동작하기때문에 Race 경합문제가 발생하지 않는다고 알고있습니다.

오버헤드가 있는걸까요? 그렇다면 어디에? 😭 혹시 어떤 이유인지 알고 계신다면 ghkdqhrbals@gmail.com 으로 연락주시면 너무너무 감사하겠습니다! 🙏