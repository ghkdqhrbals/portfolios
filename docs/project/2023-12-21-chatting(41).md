---
layout: default
title: 41. 서버 성능 개선 기록 - TPS 285% 향상
...
* 테스트 지표 확인 및 개선점
    ...
    중간중간 MTTFB 피크가 거의 **4000ms** 로 치솟으면서 서버가 안정적으로 동작하지 않는 것을 확인할 수 있죠? 그리고 이에 맞추어 TPS 또한 떨어지고, 오류도 중간에 발생하는 것을 확인할 수 있습니다.
    그래도 Error 는 0.0% 로 비교적 에러없이 동작하는 것을 확인할 수 있었습니다. TPS 는 여전히 낮고 MTTFB 또한 불안정하지만요!
...
### 5. VUSER 300 명 일 때 단일 채팅서버 파드로 핸들링 시 결과
