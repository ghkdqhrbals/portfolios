...
date: 2023-01-20
summary: "INDEX(프론트 서버 설계) 예상 UI 시퀀스 다이어그램 프론트 HTTP request/response 비동기 or Non-blocking? 예상 UI 앞서 우리는 API Gateway까지 백엔드 전체를 생성했다."
tags: ["project", "chat-server", "kafka", "index", "http", "nginx"]
nav_order: 16
---
