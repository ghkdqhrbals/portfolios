---
layout: default
title: Hotspot VM 의 JIT 컴파일러와 GraalVM ing
parent: Java-Kotlin
date: 2026-01-22
---
...
Java 는 bytecode 명령어로 이루어진 .class 까지 javac 로 컴파일링 한 뒤 JVM 위에서 한 줄씩 bytecode > binary code 로 interpreting 한다. 반복하는 곳을 런타임 때 바이너리 코드로 미리 변환해두면 성능이 더 좋아질 것이라는 아이디어에서 JIT 컴파일러가 등장했다. JIT 컴파일러는 자주 실행되는 바이트코드를 감지하여 네이티브 머신 코드로 변환하고, 이후 동일한 바이트코드가 실행될 때는 미리 변환된 네이티브 바이너리 코드를 사용하여 성능을 향상시킨다. 이 JIT 은 Java HotSpot VM 내부에 포함된 컴파일러로, "런타임 시점"에 바이트코드를 분석하여 자주 호출되는 메서드를 식별하고, 해당 메서드들을 네이티브 코드로 변환하여 캐싱한다.

여기서 Java HotSpot VM 은 JVM 구현체고 openJDK, oracle 에서 이를 사용한다. [https://en.wikipedia.org/wiki/List_of_Java_virtual_machines](https://en.wikipedia.org/wiki/List_of_Java_virtual_machines) 해당 VM 의 주 목적은 어떤 메소드가 자주 실행되는지 모니터링하면서 캐싱하고 최적화한다. [https://www.oracle.com/technical-resources/articles/java/architect-evans-pt1.html](https://www.oracle.com/technical-resources/articles/java/architect-evans-pt1.html)

JIT 컴파일러는 c1, c2 두 가지 컴파일링을 진행하는데 c1 은 short term, c2 는 long running 용 jit 컴파일러다. 보통 GUI 나 초기 캐싱이 필요한 작업의 경우 c1 에 의해 최적화 되고, 서버 사이드 작업의 경우 c2 에 의해 최적화 된다.
...
level 은 Tiered Compilation 으로 그냥 단계별로 프로파일링을 구체화해서 컴파일링 하는것이다. level 0 은 바이너리 코드 생성 없이 인터프리팅만 수행, level 1~3 까진 C1 컴파일러로 프로파일링을 구체화한다. level 4 는 C2 컴파일링 하고 더이상 승격을 위한 프로파일링을 하지 않는다. [https://devblogs.microsoft.com/java/how-tiered-compilation-works-in-openjdk/](https://devblogs.microsoft.com/java/how-tiered-compilation-works-in-openjdk/)

요약하면 말하면 Hotspot VM 은 내부에 Jit compiler 가지고 있고 최적화를 위해 해당 바이트 코드를 언제 네이티브 코드로 변환할지 tiered compilation 을 통해 결정하게 된다.

# GraalVM

앞서서 JIT 으로 다이나믹하게 컴파일링하여 캐싱하는 것을 확인하였다. **GraalVM 은 JVM 위에서 JIT 대신 c2 컴파일러로도 사용되고 AOT 컴파일러로도 사용된다.** AOT 로 사용할 시 JVM 이 필요가 없으니 런타임 오버헤드가 줄어들고, 시작 속도도 빨라지며 메모리 사용량도 감소하는 장점이 있다. 다만 JIT 의 다이나믹한 최적화가 불가능하니 성능이 떨어질 수 있다. 최신 자료는 아니지만 2019 년 기준 GraalVM AOT vs Hotspot JIT 성능 비교 자료는 아래와 같다. 처리량이 높아질 수록 GraalVM 의 최적화가 약간 떨어지는 것을 볼 수 있다. 여기서 PGO(미리 실행해보고 수집된 프로파일가지고 native image 재빌드)를 수행하면 어느정도 극복이 가능하다고 한다.

![](../2026-01-22-18-25-08.png)
* reference [Cloud Native Java: GraalVM](https://www.regist-event.com/oracle/2019/mcd_down/pdf/Day1_Track4/Day1_T4_S1.%20Cloud%20Native%20Java%20GraalVM%20-%20%EA%B9%80%ED%83%9C%EC%99%84%20%EB%B6%80%EC%9E%A5.pdf)

**확실한건 memory 사용이 줄고 시작속도가 빨라진다는 점이다.** 안그래도 Java 서비스들은 JVM 이라는 런타임 오버헤드가 있어서 시작속도가 느리고 메모리 사용량이 많았는데 GraalVM 으로 빌드하면 이 문제를 어느정도 해결할 수 있다. 작은 프로그램 띄우기 매우 좋다고 볼 수 있다.