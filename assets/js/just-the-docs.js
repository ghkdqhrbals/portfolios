...
// Modify image paths in Markdown content
          function modifyImagePaths() {
            const images = document.querySelectorAll('img');
            images.forEach(image => {
              const src = image.getAttribute('src');
              if (src && !src.startsWith('/portfolios/assets/')) {
                image.setAttribute('src', '/portfolios/assets/' + src);
              }
            });
          }
...
// Document ready
          ...
          jtd.onReady(function(){
            initNav();
            modifyImagePaths(); // Call the function to modify image paths
            {%- if site.search_enabled != false %}
            initSearch();
            {%- endif %}
            scrollNav();
